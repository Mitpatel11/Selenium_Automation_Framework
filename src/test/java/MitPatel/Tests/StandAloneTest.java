...
public class StandAloneTest {
	...
	public static void main(String[] args) {
		...
		driver.findElement(By.id("userPassword")).sendKeys("hNKurJ#!3CV.3G9");
		driver.findElement(By.id("login")).click();

		List<WebElement> products = driver.findElements(By.cssSelector(".mb-3"));

		// here as we are using the single product as webelement the scope of the
		// webelement is fixed so we can directly use the tagname and it will not check
		// the full webpage
		// also find first will only check the first filtered result and not all the
		// multiple products
		WebElement reqprod = products.stream()
				.filter(product -> product.findElement(By.tagName("b")).getText().equals(productName)).findFirst()
				.orElse(null);
		reqprod.findElement(By.cssSelector(".card-body button:last-of-type")).click();
